...
namespace Neptuno2022EF.Windows
{
    public partial class frmVentaAE : Form
    {
        ...
        public frmVentaAE(IServiciosClientes servicioCliente, IServiciosProductos servicioProductos, IServiciosCtasCtes servicioCtasCtes)
        {
            InitializeComponent();
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            if (ValidarVenta())
            {
                ...
                var cta = _servicioCtasCtes.GetCtaCtePorClienteId(venta.ClienteId);
                if (_servicioCtasCtes.Existe(ctaCorriente))
                {
                    ctaCte = new CtaCte()
                    {
                        ClienteId = cliente.Id,
                        NumeroFactura = $"Fact-{_servicioCtasCtes.GetCantidad()}",
